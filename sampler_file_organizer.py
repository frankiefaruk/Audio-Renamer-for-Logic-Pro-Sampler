...
class AudioFileRenamerTextFileGenerator:
    def update_info(self):
        """Updates the information text based on selected action"""
        info_texts = {
            "rename_midi": """• Scans the selected folder for audio files
• Extracts the MIDI note name from each filename
• Sorts the files based on their MIDI note number
• Renames each file with a new filename format that includes:
  - MIDI note number
  - Original filename components
  - Velocity information""",
            
            "rename": """• Creates new audio files based on the specified base name and MIDI note mapping
• Uses the MIDI note number and note name to generate unique filenames for each new file""",
            
            "text": """• Generates a text file containing a list of filenames based on the specified base name and MIDI note mapping
• Each line in the text file represents a potential audio filename"""
        }
        
        selected_action = self.action_var.get()
        self.info_label.config(text=info_texts.get(selected_action, ""))
    ...
    def __init__(self, root):
        ...
        # Action selection
        self.action_var = tk.StringVar(value="rename_midi")
        tk.Radiobutton(main_frame, text="Rename Files by MIDI Order", 
        ...
        tk.Radiobutton(main_frame, text="Rename Audio Files", 
                      variable=self.action_var, 
                      value="rename", 
                      command=self.update_info,
                      bg="#323232", fg="white", 
                      selectcolor="#4D4D4D", 
                      font=label_font).pack(anchor='w')
        tk.Radiobutton(main_frame, text="Create Text File", 
                      variable=self.action_var, 
                      value="text", 
                      command=self.update_info,
                      bg="#323232", fg="white", 
        ...
        # Add separator
        separator = tk.Frame(main_frame, height=2, bg="#4D4D4D")
        separator.pack(fill=tk.X, pady=10)

        # Information frame
        info_frame = tk.LabelFrame(main_frame, text="Information", bg="#323232", fg="white",
                                   font=label_font, bd=0, relief="flat")
        info_frame.pack(fill=tk.X, pady=10, padx=5)
        
        self.info_label = tk.Label(info_frame, text="", bg="#323232", fg="white", 
                                  font=label_font, wraplength=460, justify=tk.LEFT)
    ...
    def update_preview(self):
        ...
        if action == "rename_midi":
            ...
            preview_text = "Preview of MIDI order renaming:\n\n"
            self.rename_pairs = []
            
            for filename in sorted(files):
                try:
                    # Extract note name using regex
                    note_match = re.search(r'[A-G]#?\-?\d+', filename)
                    if not note_match:
                        preview_text += f"Skipping {filename} (no note found)\n"
                        continue
                        
                    note_name = note_match.group(0)
                    # Handle cases where note might have a hyphen (e.g., C-1)
                    if '-' in note_name and note_name not in self.note_mapping:
                        note_name = note_name.replace('-', '')
                    
                    midi_num = self.note_mapping.get(note_name)
                    if midi_num is None:
                        preview_text += f"Skipping {filename} (invalid note: {note_name})\n"
                        continue

                    # Split filename and extract components
                    name_parts = filename.split('-')
                    first_part = name_parts[0].split('_')  # Split the first part by underscores
                    
                    # Extract base name and group (RR1)
                    base_name = first_part[0] + "_" + first_part[1]  # Bass_Gadulka
                    group = first_part[2]  # RR1
                    
                    # Extract velocity
                    velocity_match = re.search(r'V\d+(?:-\d+)?', first_part[3])
                    velocity = velocity_match.group(0) if velocity_match else "V127"
                    
                    ext = os.path.splitext(filename)[1]
                    
                    # Create new name with desired order:
                    # [MIDI]_[Note]_[BaseName]_[Velocity]_[Group]
                    new_name = f"{midi_num}_{note_name}_{base_name}_{velocity}_{group}{ext}"
                    
                    preview_text += f"{filename} → {new_name}\n"
                    
                    old_path = os.path.join(self.selected_folder, filename)
                    new_path = os.path.join(self.selected_folder, new_name)
                    self.rename_pairs.append((old_path, new_path))
                    
                except Exception as e:
                    preview_text += f"Error processing {filename}: {str(e)}\n"
    ...
    def submit_rename(self):
        ...
        if action == "rename_midi":
            message = "This will prefix all files with their MIDI note numbers. Proceed?"
        ...
        if messagebox.askyesno("Confirm Rename", message):
            try:
                ...
                messagebox.showerror("Error", f"An error occurred while renaming: {str(e)}")
